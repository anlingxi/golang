...
func (s *userService) RefreshToken(refreshTokenString string) (newAccessToken, newRefreshToken string, err error) {
	...
	// ✅ 新增：检查 refresh 是否已被拉黑（rotation）
	// 防止 refresh token 被盗用后持续使用，必须确保每次刷新后旧的 refresh token 都被拉黑。
	if claims.ID != "" {
		v, err := database.RDB.Exists(context.Background(), "blacklist:refresh:"+claims.ID).Result()
		if err == nil && v == 1 {
			return "", "", errors.New("invalid refresh token")
		}
	}

	// ✅ 新增：确保只能用 refresh token 刷新
	...
	// ✅ 新增：refresh rotation —— 拉黑旧 refresh 的 jti
	if claims.ID != "" && claims.ExpiresAt != nil {
		expiration := time.Until(claims.ExpiresAt.Time)
		if expiration > 0 {
			_ = database.RDB.Set(context.Background(), "blacklist:refresh:"+claims.ID, "true", expiration).Err()
		}
	}
